...
export interface CalculatorPanelProps {
  initialSystem?: GradingSystem;
  title?: string;
  subtitle?: string;
  showReportButton?: boolean;
}

export const CalculatorPanel = memo(({ 
  initialSystem = 'USA',
  title = "Subjects Panel",
  subtitle = "Workspace",
  showReportButton = true
}: CalculatorPanelProps) => {
  const [gradingSystem, setGradingSystem] = useState<GradingSystem>(initialSystem);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setGradingSystem(data.gradingSystem || initialSystem);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8 items-start w-full">
      ...
      <div className="space-y-6 w-full">
        <GlassCard className="p-6 md:p-10">
          <div className="mb-10 text-center md:text-left">
            <h3 className="text-xs font-bold text-primary uppercase tracking-[0.2em] mb-2">{subtitle}</h3>
            <h2 className="text-2xl md:text-3xl font-extrabold font-display leading-none">{title}</h2>
          </div>
      ...
      <div className="space-y-6 lg:sticky lg:top-32 w-full lg:max-w-none">
                     ...
                     <svg className="w-full h-full transform -rotate-90 absolute p-4">
                       <circle cx="50%" cy="50%" r="44%" stroke="#f3f4f6" strokeWidth="8" fill="transparent" />
                        <motion.circle 
                          ...
                          cx="50%" cy="50%" r="44%" stroke="url(#gradient-calc)" strokeWidth="10" fill="transparent" 
                          strokeDasharray="452.38"
                          strokeLinecap="round"
                        />
                      <defs>
                        <linearGradient id="gradient-calc" x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        <section className="p-6 bg-gradient-to-br from-primary to-secondary border-none text-white flex items-center justify-between group cursor-pointer overflow-hidden relative rounded-[24px]">
