...
export const Navbar = memo(({ darkMode, setDarkMode }: { darkMode: boolean; setDarkMode: (v: boolean) => void }) => {
  ...
  return (
    <nav className="fixed top-4 md:top-8 left-4 right-4 md:left-6 md:right-6 z-50 bg-white/40 backdrop-blur-2xl rounded-[16px] md:rounded-[20px] shadow-2xl shadow-primary/10 border border-white/60">
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-14 md:h-18 flex items-center justify-between gap-2">
        ...
        <div className="hidden lg:flex items-center gap-8 text-sm font-bold text-gray-600">
          ...
          <button onClick={() => scrollToSection('global-standards')} className="hover:text-primary transition-colors cursor-pointer">Standards</button>
